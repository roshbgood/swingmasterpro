...
import { Shield, TrendingUp, Activity, Target, BookOpen, Zap, PlusCircle, Maximize2, Layers } from 'lucide-react';

const RiskManagementSystem: React.FC = () => {
  const [activeStrategy, setActiveStrategy] = useState<'3-STOP' | '2-STOP'>('3-STOP');
    ...
    <div className="max-w-6xl mx-auto space-y-8 pb-20">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            ...
            Systematic approaches to stops, profit taking, and management.
            </p>
        </div>
        <div className="bg-slate-900 p-1 rounded-lg border border-slate-700 flex gap-1">
            <button
                onClick={() => setActiveStrategy('3-STOP')}
                className={`px-4 py-2 rounded-md text-sm font-bold transition-all ${activeStrategy === '3-STOP' ? 'bg-emerald-600 text-white shadow' : 'text-slate-400 hover:text-white hover:bg-slate-800'}`}
            >
                3-Stop System
            </button>
            <button
                onClick={() => setActiveStrategy('2-STOP')}
                className={`px-4 py-2 rounded-md text-sm font-bold transition-all ${activeStrategy === '2-STOP' ? 'bg-emerald-600 text-white shadow' : 'text-slate-400 hover:text-white hover:bg-slate-800'}`}
            >
                2-Stop System
            </button>
        </div>
      ...
      <div className="bg-slate-800 rounded-xl border border-slate-700 p-4 overflow-hidden shadow-lg">
         <div className="flex justify-between items-center mb-4 px-2">
            <h3 className="text-sm font-bold text-slate-400 uppercase tracking-wider">
                {activeStrategy === '3-STOP' ? 'Jeff Sun\'s 3-Stop Timeline' : 'Standard 2-Stop Timeline'}
            </h3>
            <span className="text-xs text-slate-500">Day 0 to Day 4+</span>
         </div>
         <div className="w-full overflow-x-auto pb-2 custom-scrollbar">
           {activeStrategy === '3-STOP' ? (
               ...
               <svg viewBox="0 0 800 160" className="w-full min-w-[700px] h-auto font-sans">
                  {/* Arrow Lines 2-Stop */}
                  <line x1="200" y1="70" x2="280" y2="70" stroke="#475569" strokeWidth="2" markerEnd="url(#arrow)" />
                  <line x1="480" y1="70" x2="560" y2="70" stroke="#475569" strokeWidth="2" markerEnd="url(#arrow)" />
                  ...
                  {/* Node 1: Entry */}
                  <g transform="translate(0, 10)">
                     <rect width="200" height="120" rx="8" fill="#10b981" fillOpacity="0.1" stroke="#10b981" strokeWidth="2" />
                     <text x="100" y="25" textAnchor="middle" fill="#10b981" fontWeight="bold" fontSize="14">ENTRY</text>
                     <line x1="10" y1="35" x2="190" y2="35" stroke="#10b981" strokeOpacity="0.3" />
                     <text x="100" y="60" textAnchor="middle" fill="#cbd5e1" fontSize="11">2 Stops (50% ea)</text>
                     <text x="100" y="85" textAnchor="middle" fill="#94a3b8" fontSize="10">Stop 1: 50% Risk (Half R)</text>
                     <text x="100" y="100" textAnchor="middle" fill="#94a3b8" fontSize="10">Stop 2: Full Stop (LoD)</text>
                  </g>

                  {/* Node 2: First Target */}
                  <g transform="translate(280, 10)">
                     <rect width="200" height="120" rx="8" fill="#3b82f6" fillOpacity="0.1" stroke="#3b82f6" strokeWidth="2" />
                     <text x="100" y="25" textAnchor="middle" fill="#3b82f6" fontWeight="bold" fontSize="14">Profit: 2R - 3R</text>
                     <line x1="10" y1="35" x2="190" y2="35" stroke="#3b82f6" strokeOpacity="0.3" />
                     <text x="100" y="60" textAnchor="middle" fill="#cbd5e1" fontSize="11">Scale Out</text>
                     <text x="100" y="85" textAnchor="middle" fill="#ffffff" fontWeight="bold" fontSize="11">Sell 50% Position</text>
                     <text x="100" y="100" textAnchor="middle" fill="#94a3b8" fontSize="10">Move Stop 2 to Breakeven</text>
                  </g>

                   {/* Node 3: Runner */}
                   <g transform="translate(560, 10)">
                     <rect width="200" height="120" rx="8" fill="#a855f7" fillOpacity="0.1" stroke="#a855f7" strokeWidth="2" />
                     <text x="100" y="25" textAnchor="middle" fill="#a855f7" fontWeight="bold" fontSize="14">Trend Management</text>
                     <line x1="10" y1="35" x2="190" y2="35" stroke="#a855f7" strokeOpacity="0.3" />
                     <text x="100" y="60" textAnchor="middle" fill="#cbd5e1" fontSize="11">Trailing Stop</text>
                     <text x="100" y="85" textAnchor="middle" fill="#94a3b8" fontSize="10">Trail remaining 50%</text>
                     <text x="100" y="100" textAnchor="middle" fill="#ffffff" fontWeight="bold" fontSize="11">Use 10-MA or 20-MA</text>
                  </g>
               </svg>
           )}
         </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {activeStrategy === '3-STOP' ? (
            <>
                ...
                <div className="bg-slate-800 rounded-xl border border-slate-700 overflow-hidden">
                    <div className="bg-blue-900/20 p-4 border-b border-slate-700 flex items-center gap-3">
                        ...
                        <h3 className="font-bold text-white">Profit Taking (2R Rule)</h3>
                    </div>
                    <div className="p-5 space-y-3">
                        <div className="flex gap-3">
                            <TrendingUp className="h-5 w-5 text-emerald-400 shrink-0" />
                            <div>
                                <p className="text-sm text-slate-300 font-medium">Sell 1/3 Strength</p>
                                <p className="text-xs text-slate-400 mt-1">
                                    If profit exceeds <strong>2R</strong> (2x Risk), immediately sell <strong>33%</strong>. Maintain stops for remainder but adjust size.
                                </p>
                            </div>
                        </div>
                    </div>
                ...
                <div className="bg-slate-800 rounded-xl border border-slate-700 overflow-hidden">
                    <div className="bg-orange-900/20 p-4 border-b border-slate-700 flex items-center gap-3">
                        ...
                        <h3 className="font-bold text-white">Time Stop / Adjustment</h3>
                    </div>
                    <div className="p-5 space-y-3">
                        <div className="flex gap-3">
                            <Target className="h-5 w-5 text-orange-400 shrink-0" />
                            <div>
                                <p className="text-sm text-slate-300 font-medium">Size Down & Breakeven</p>
                                <p className="text-xs text-slate-400 mt-1">
                                    Sell <strong>33%</strong> if not already taken. Consolidate ALL remaining stops to <strong>Breakeven</strong>.
                                </p>
                            </div>
                        </div>
                    </div>
            ...
            <>
                ...
                <div className="bg-slate-800 rounded-xl border border-slate-700 overflow-hidden">
                    <div className="bg-cyan-900/20 p-4 border-b border-slate-700 flex items-center gap-3">
                        <div className="bg-cyan-500 text-white font-bold px-2 py-1 rounded text-xs">SETUP</div>
                        <h3 className="font-bold text-white">2-Stop Structure</h3>
                    </div>
                    <div className="p-5 space-y-3">
                        <div className="flex gap-3">
                            <Layers className="h-5 w-5 text-cyan-400 shrink-0" />
                            <div>
                                <p className="text-sm text-slate-300 font-medium">Aggressive vs Structural</p>
                                <p className="text-xs text-slate-400 mt-1">
                                    Split position 50/50. 
                                    <br/>• Stop 1 (Aggressive): 50% risk distance. Protects against immediate failure.
                                    <br/>• Stop 2 (Structural): Low of Day or Technical Pivot.
                                </p>
                            </div>
                        </div>
                    </div>
                ...
                <div className="bg-slate-800 rounded-xl border border-slate-700 overflow-hidden">
                    <div className="bg-blue-900/20 p-4 border-b border-slate-700 flex items-center gap-3">
                        <div className="bg-blue-500 text-white font-bold px-2 py-1 rounded text-xs">TARGETS</div>
                        <h3 className="font-bold text-white">Scaling Out</h3>
                    </div>
                    <div className="p-5 space-y-3">
                        <div className="flex gap-3">
                            <Target className="h-5 w-5 text-emerald-400 shrink-0" />
                            <div>
                                <p className="text-sm text-slate-300 font-medium">Primary Target</p>
                                <p className="text-xs text-slate-400 mt-1">
                                    Aim for 2R to 3R on the first half. Once hit, move the stop on the second half to Breakeven to guarantee a risk-free trade.
                                </p>
                            </div>
                        </div>
                    </div>
                 ...
                 <div className="bg-slate-800 rounded-xl border border-slate-700 overflow-hidden">
                    <div className="bg-purple-900/20 p-4 border-b border-slate-700 flex items-center gap-3">
                        <div className="bg-purple-500 text-white font-bold px-2 py-1 rounded text-xs">RUNNER</div>
                        <h3 className="font-bold text-white">Trend Following</h3>
                    </div>
                    <div className="p-5 space-y-3">
                        <div className="flex gap-3">
                            <TrendingUp className="h-5 w-5 text-purple-400 shrink-0" />
                            <div>
                                <p className="text-sm text-slate-300 font-medium">Catch the Move</p>
                                <p className="text-xs text-slate-400 mt-1">
                                    Let the remaining 50% run using a trailing stop (e.g. 10-day or 20-day Moving Average) to maximize gains on big winners.
                                </p>
                            </div>
                        </div>
                    </div>
